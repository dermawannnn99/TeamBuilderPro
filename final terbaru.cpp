...
void tampilkanSemua() {
    ...
    // Salin dan urutkan berdasarkan IPK
    ...
    for (size_t i = 0; i < sorted.size(); ++i) {
        cout << left << setw(5) << i + 1 << setw(20) << sorted[i].nama
             << setw(15) << sorted[i].nim << setw(20) << sorted[i].jurusan
             << setw(5) << fixed << setprecision(2) << sorted[i].ipk << endl;
    }
}
...
void bagiKelompokAcak() {
    ...
    // Sortir mahasiswa berdasarkan IPK
    vector<Mahasiswa> sorted = daftarMahasiswa;
    sort(sorted.begin(), sorted.end(), [](const Mahasiswa& a, const Mahasiswa& b) {
        return a.ipk > b.ipk;
    });
    ...
    // Bagi ke dalam 3 pot
    vector<vector<Mahasiswa>> pots(3);
    int size = sorted.size();
    int perPot = size / 3 + (size % 3 != 0); // Pembagian merata
    for (int i = 0; i < size; ++i) {
        pots[i / perPot].push_back(sorted[i]);
    }
    ...
    // Bagi dari setiap pot ke kelompok (agar setiap kelompok mendapat dari setiap pot)
    for (auto& pot : pots) {
        shuffle(pot.begin(), pot.end(), g);
        for (size_t i = 0; i < pot.size(); ++i) {
            int kelompokKe = (i % jumlahKelompok) + 1;
            pembagianKelompok[kelompokKe].push_back(pot[i]);
        }
    }
